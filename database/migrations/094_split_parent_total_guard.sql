-- 094: enforce the split-parent invariant  transactions.amount = SUM(transaction_splits.amount)
--
-- Why this exists
-- ---------------
...
-- (updateTransferFromSplit, which re-derives the parent from its splits and
-- never writes leg fields onto it). This migration is the backstop, so any
-- *future* code path that breaks the invariant fails loudly instead of quietly
